...
def zed1_image_cv(msg):
	global image_np_global, depth_np_global, exit_signal, new_data
	# convert ros image to opencv image. copy needed in order to make array mutable.
	image_mat = np.copy(self.bridge.imgmsg_to_cv2(msg, desired_encoding="passthrough"))
	image_np_global = load_image_into_numpy_array(image_mat)
	new_data = True
	return

def zed_depth_cv(msg):
	global image_np_global, depth_np_global, exit_signal, new_data
...
def main(args):
	...
	bridge = CvBridge()
	rospy.Subscriber('/zed/left/image_rect_color', Image, self.zed1_image_cb, queue_size=50)
	rospy.Subscriber('/zed/depth/depth_registered', Image, self.zed1_depth_cb, queue_size=50)
